...
export default function EventPage() {
	...
	const {
		...
		event ? `/api/posts?event=${encodeURIComponent(event)}&limit=${POSTS_PER_PAGE}&offset=${postsPage * POSTS_PER_PAGE}` : null,
		fetcher,
		{
	...
	// eventが変更されたら、投稿リストをリセット
	useEffect(() => {
		setPostsPage(0);
		setAllPosts([]);
		setHasMorePosts(true);
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [rawEvent]); // rawEventが変更されたらリセット
